/** SHA-256 of UTF-8 bytes. Sync so ingest can refuse before storing. */
...
function pad(message: Uint8Array): Uint8Array {
  ...
  const out = new Uint8Array(withOne + padLen + 8);
  out.set(message);
  out[message.length] = 0x80;
  const view = new DataView(out.buffer);
  const high = Math.floor(bitLen / 0x100000000);
  view.setUint32(out.length - 8, high);
  view.setUint32(out.length - 4, bitLen >>> 0);
  return out;
}
